...
GITHUB_REPO = os.getenv("GITHUB_REPO")  # repo to create the tickets, format: "username/repo"
api_key = os.getenv("OPENAI_API_KEY")  # get the key from .env

client = OpenAI(api_key=api_key)  # define openai object
...
tools = [
    {
        "type": "function",
        "name": "query_database",  # tool to ask database
        "description":
    ...
    {
        "type": "function",
        "name": "create_github_ticket",  # tool to create ticket
        "description":
...
def create_github_ticket(title: str, description: str, labels):
    if not GITHUB_TOKEN or not GITHUB_REPO:  # check GitHub token and repo name
    ...
    response = requests.post(url, json=data, headers=headers)  # make request to GitHub API

    if response.status_code == 201:  # if response successful return issue url
...
def query_database(query: str):
    ...
    try:
        cursor.execute(query)  # execute generated query
        rows = cursor.fetchall()  # get all data from response
        return {"sample_rows": rows}
    except Exception as ex:  # if error occurred return error message
        return {"error": str(ex)}
    finally:
...
def run_agent(user_input, chat_history=None):
    ...
    # system prompt to control the agent’s behavior
    system_prompt = {
        "role": "system",
        "content": "You are a helpful data insight assistant for a movie database. "
                   "Your role is to help users get insights from movie data."
    }
    ...
    for item in response.output:
        if item.type == "function_call":
            if item.name == "query_database":
                ...
                result = query_database(args["query"])  # call function
            elif item.name == "create_github_ticket":
                st.session_state.logs.append(f"[{datetime.now().strftime('%H:%M:%S')}] Function call used, function - create_github_ticket")
                args = json.loads(item.arguments)
                result = create_github_ticket(args["title"], args["description"], args.get("labels", []))  # call function
    ...
    # ssk model to generate final user-facing response
    ...
    st.session_state.logs.append(
        f"[{datetime.now().strftime('%H:%M:%S')}] Final response generated")
